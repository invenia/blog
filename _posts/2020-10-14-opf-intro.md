...
\left\{
	\begin{aligned}
		...
		q_{ij} & = b_{ij} \left( -v_{i}^{2} + v_{i} v_{j} \sin\delta_{ij} \right) - g_{ij} \left( v_{i} v_{j} \cos\delta_{ij} \right) \\
	\end{aligned}
\right.
\label{power_flow_y}
\end{equation}
...
UC is therefore a mixed-integer programming that is an even more expensive NP-hard problem.
...
However, solving such problems by interior-point method is not cheap as it requires the computation of the Hessian of the Lagrangian ($$\nabla_{xx} L(x_{n}, s_{n}, y_{n}, z_{n})$$) at each step $$n$$.
