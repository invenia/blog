...
Power grids use alternating current (AC) so we start with some notations and definitions of AC curcuits.
...
Eq. $$\ref{power_balance}$$ is a conservation low of the power (energy).
...
Although the formulations are equivalent in practice we might prefer one model over the other: depending on the actual problem and the structure of system it might be easier to obtain results and derive exact or approximate relaxation from one formulation than the other one.
...
Below we will constract more sophisticated models by extending the basic equations considering the followings:
- multiple generators and loads included by buses
- shunt elements connected to buses
- modelling transformers and phase shifters
- modelling asymmetric line charging
...
Also, line charging is usually modelled by the $$\pi$$ transmission line (or $$\pi$$-section) model that places shunt admittances at the two ends of the transmission line beside its series admittance.
...
Optimal power flow (OPF) is a constrained optimization problem that is based on the power flow problem.
The most important ingredients of the model are: its *variables* that minimize an *objective function*, subject to some *equality constrains* inlcuding the power balance and power flow equations and *inequality constraints* including bounds on the variables.
...
Then we provide a brief overview of the different basic types of OPF.
We will see that - unlike PF - OPF is in general rather expensive to solve so finally we also discuss approximations that simplify the original model (relaxation).
...
$$C_{g}$$ is a monotonically increasing function of $$p_{g}^{G}$$, usually modelled by a quadratic or piecewise linear function.
The objective function, $$C(X)$$, then can be written as:

$$C(X) = \sum \limits_{g \in \mathcal{G}} C_{g}(p_{g}^{G})$$

We note that there can be alternative objective functions to minimize in OPF that can be used to investigate different features of the power grid.
...
An example, with the increasing concerns over climate change, multiple studies have suggested that reducing overall emissions from generation should be also taken into account in the objective function[^Gholami14].
...
In a more general setting there can be multiple reference nodes and corresponding contraints on their voltage angle.
...
Finally, OPFs also include a set of inequality constraints on the variables that usually express some physical limitations in the system. 
...
#### Solving the ED problem by interior-point method
----------------------------------------------------

Economic dispatch is non-linear non-convex optimization problem.
One of the most successful techniques to solve such large-scale programming is interior-point or barrier methods.[^Nocedal06] [^Wachter06]
...
However, as $$\mu \to 0$$ the solution of the barrier problem approaches that of the problem $$\ref{opt_problem}$$.
...
where the $$J_{\mathrm{E}}(x)$$ and $$J_{\mathrm{I}}(x)$$ are Jacobian matrices of the equality and ineqaulity functions, respectively, and
...
Therefore, it is pretty desirable to allow shutting down specific generators in the optimization model.
In order to select the actual generators electricity grid operators solve the *unit commitment* (UC) problem.
...
UC is therefore a mixed-integer programming that is an even more expensive problem.

Power grids are complex systems that include thousands of buses and transmission lines, where failure of any elements can happen occasionally.
...
These problems are therefore much larger scale problems including multiple of variables and constraints of the ED problem.
For example, the widely used $$N-1$$ contingency problem addresses all scenarios where a single bus fails.
...
Moreover, the scaling is even worse for OPF problems that lead to mixed-integer programming.
...
where $$M_{k}$$s are Hermitian matrices and $$m_{k}$$s are real valued vectors specifying the corresponding inequality constraints.
...
where $$\mathbb{S}^{N}$$ denotes the space of $$N \times N$$ symmetric matrices, $$\mathrm{tr}$$ and $$\mathrm{rank}$$ are the trace and rank functions, repsectively.
