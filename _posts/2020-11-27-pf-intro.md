...
Power grids generally use alternating current (AC), so we start with some notation and definitions for AC circuits.
...
First, let us consider the BIM equations (eq. $$\ref{bim_basic_concise}$$) that define a complex non-linear system with $$N = |\mathcal{N}|$$ complex equations, and $$2N$$ complex variables, $$\left\{S_{i}, V_{i}\right\}_{i=1}^{N}$$.
...
The BFM formulation (eq. $$\ref{bfm_basic_concise}$$) includes $$N + 2E$$ complex equations with $$2N + 2E$$ complex variables: $$\left\{ S_{i}, V_{i} \right\}_{i \in \mathcal{N}} \cup \left\{ S_{ij}, I_{ij} \right\}_{(i, j) \in \mathcal{E}}$$ or equivalently, $$2N + 4E$$ real equations with $$4N + 4E$$ real variables.
...
Finally, the series current has the following simple expression:  $$I_{ij}^{s} = Y_{ij} \left( \frac{V_{i}}{T_{ij}} - V_{j} \right)$$.
